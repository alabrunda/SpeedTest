...
namespace SpeedTest.Client.Model
{
    public class SpeedTestClient : ISpeedTestClient
    {
        ...
        public static async void testmessage(string message)
        {
            var result = await new HttpClient().GetAsync("http://" + message);
            //Console.WriteLine(result.StatusCode);//Don't care about result.
        }

        public static async Task SendSpeedResult(SpeedResult speedResults, string URL)
        {
            var json = JsonConvert.SerializeObject(speedResults);
            System.Console.WriteLine("Sending: " + json);
            var data = new StringContent(json, Encoding.UTF8, "application/json");
            var url = URL;
            using var client = new HttpClient();
            var response = await client.PostAsync(url, data);
            //System.Console.WriteLine("ResultCode: " + response.StatusCode);
            string result = response.Content.ReadAsStringAsync().Result;            
        }

        public static async Task ValidateAPIAccess(string URL)
        {
            using var client = new HttpClient();
